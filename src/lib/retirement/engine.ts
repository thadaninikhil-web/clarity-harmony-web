...
export function validateInputs(input: RetirementInputs): ValidationResult {
  ...
  for (const [k, label] of finiteFields) {
    ...
    if (!Number.isFinite(v)) {
      errors.push({
        field: label,
        rule: `${label} must be a finite number`,
        message: `${label} is missing or non-numeric.`,
      });
    }
  }
...
export function projectTwoBucket(rawInput: RetirementInputs): ProjectionResult {
  ...
  for (let y = 1; y <= totalYears; y++) {
    ...
    if (input.stressEnabled) {
      if (input.stressMode === "crash") {
        ...
        const inStress =
          y >= input.crashYearOffset && y < input.crashYearOffset + input.recoveryYears + 1;
    ...
    if (!isRetired) {
      ...
      const annualExpense =
        ...
        12 *
        Math.pow(1 + input.inflationRate, yearsToRetirement + yrsSinceRetire);
      ...
      if (remaining > 0) {
        ...
        if (fromEquity > 0)
          notes.push(`Drew ${formatINR(fromEquity)} from equity sleeve`);
      }
    ...
    rows.push({
      ...
      accOpening: equityOpening,
      accGrowth,
      prepOpening: 0,
      prepGrowth: 0,
      withdOpening: debtOpening,
      withdGrowth,
      emergencyReserve,
      emergencyUsed,
  ...
  return {
    ...
    monthlyExpenseAtRetirement:
      input.currentMonthlyExpenses * Math.pow(1 + input.inflationRate, yearsToRetirement),
    emergencyFundAtRetirement: emergencyAtRetirement,
    depleted,
    depletionAge,
    finalCorpus: rows[rows.length - 1].total,
    emergencyUsedFirstYear,
    emergencyUsedFirstAge,
    emergencyUsedTotal,
  };
}
    ...
    if (!isRetired) {
      ...
      if (yearsLeft >= 0 && yearsLeft < input.prepYearsBeforeRetirement) {
        ...
        const pvOfGap =
          yearsToGrow === 0
            ? fvGap
            : fvGap / Math.pow(1 + input.prepReturn, yearsToGrow);
        ...
        if (moveThisYear > 0)
          notes.push(`Glide-path: moved ${formatINR(moveThisYear)} Acc → Prep`);
      } else {
        prep = prep * (1 + input.prepReturn);
      }
      ...
      if (yrsSinceRetire === 0) {
        ...
        notes.push(
          `Funded Withdrawal with ${formatINR(target)} (${input.withdrawalYears} yrs of expenses + ${formatINR(emergencyReserve)} emergency reserve)`,
        );
      }
...

      {
        ...
        const withdTargetAfterSpend =
          nextExpense * input.withdrawalYears + emergencyReserve;
        const refillW = Math.min(prep, Math.max(0, withdTargetAfterSpend - withd));
        prep -= refillW;
        withd += refillW;
        transfers.prepToWithd += refillW;
        if (refillW > 0)
          notes.push(`Refilled Withdrawal with ${formatINR(refillW)} from Prep`);
        ...
        if (short > 0) {
          ...
          short -= extraFromW;
          if (extraFromW > 0)
            notes.push(`Shortfall covered from Withdrawal: ${formatINR(extraFromW)}`);
        }
        ...
        if (refillP > 0)
          notes.push(`Refilled Preparation with ${formatINR(refillP)} from Acc`);
